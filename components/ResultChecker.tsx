    ...
    <div className="max-w-2xl mx-auto space-y-8 py-4">
      ...
      <div className="mt-16 animate-in slide-in-from-bottom duration-700">
        <div className="text-center mb-8">
          ...
          <h3 className="text-2xl font-kids text-indigo-600 mt-2">รายชื่อคนส่งงาน & ประกาศคะแนน</h3>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Submission List & Scoreboard</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-indigo-50 border-b-2 border-indigo-100">
                  <th className="p-4 text-left text-indigo-600 font-bold text-sm uppercase tracking-wider pl-8">ลำดับ / ชื่อ</th>
                  ...
                  <th className="p-4 text-center text-indigo-600 font-bold text-sm uppercase tracking-wider pr-8">สถานะ / คะแนน</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-indigo-50">
                {submissions
                  .sort((a, b) => {
                    // 1. Graded first
                    const statusA = a.review?.status === 'Graded' ? 1 : 0;
                    const statusB = b.review?.status === 'Graded' ? 1 : 0;
                    if (statusA !== statusB) return statusB - statusA;

                    // 2. If Graded, sort by Score Desc
                    if (statusA === 1) {
                      return (b.review?.totalScore || 0) - (a.review?.totalScore || 0);
                    }

                    // 3. If Pending, sort by ID Desc (Newest first)
                    return (b.rowId || 0) - (a.rowId || 0);
                  })
                  .map((s, index) => {
                    const isGraded = s.review?.status === 'Graded';
                    return (
                      <tr key={`${s.sheetName}-${s.rowId}`} className={`transition-colors ${isGraded ? 'hover:bg-green-50/30' : 'hover:bg-yellow-50/30 bg-slate-50/50'}`}>
                        <td className="p-4 pl-8">
                          <div className="flex items-center gap-3">
                            {isGraded ? (
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm ${index < 3 ? 'bg-yellow-400 ring-2 ring-yellow-200' : 'bg-green-400'}`}>
                                {index + 1}
                              </div>
                            ) : (
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-slate-400 bg-slate-200">
                                -
                              </div>
                            )}
                            <span className={`font-bold ${isGraded ? 'text-slate-700' : 'text-slate-500'}`}>{s.name}</span>
                          </div>
                        </td>
                        ...
                        <td className="p-4 text-center">
                          <span className="text-xl" title={s.activityType}>{s.activityType === 'Sports Day' ? '🏃' : '🎈'}</span>
                        </td>
                        <td className="p-4 text-center pr-8">
                          {isGraded ? (
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <span className="text-xs text-slate-300 font-bold">/20</span>
                            </div>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-yellow-100 text-yellow-600 text-xs font-bold animate-pulse">
                              ⏳ รอตรวจ
                            </span>
                          )}
                        </td>
                ...
                {submissions.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-12 text-center text-slate-400 font-bold italic">
                      ยังไม่มีข้อมูลการส่งงานจ้า... เริ่มส่งงานเป็นคนแรกเลย! 🚀
                    </td>
                  </tr>
                )}
              </tbody>
